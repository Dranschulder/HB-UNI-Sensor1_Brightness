...
public:
    void init(uint8_t msgcnt, int16_t temp, uint16_t airPressure, uint8_t humidity, uint32_t brightness, uint8_t digInputState,
              uint16_t batteryVoltage, bool batLow, int16_t customData)
    {
...
class WeatherChannel : public Channel<Hal, List1, EmptyList, List4, PEERS_PER_CHANNEL, SensorList0>, public Alarm {
    ...
    int16_t customData; // Temperatur des DS18X20 (als 2. Temperatur; denn es kann nur eine übergeben werden)
...
public:
    ...
    && !defined(SENSOR_SHT10)
        temperature10 = -1000;    // 18.8C (scaling 10)
#endif
        ...
        airPressure10 = 0;    // 1088 hPa (scaling 10)
#endif
